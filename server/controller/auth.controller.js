...
// Safe cross-site cookie configuration
const cookieOptions = (maxAge) => ({
  httpOnly: true,
  secure: true,      // Hamesha true kyunki Vercel HTTPS use karta hai
  sameSite: "none",  // Cross-site cookie movement ke liye zaroori hai
  maxAge,
});
...
export const refreshTokens = async (req, res) => {
  try {
    ...
    if (!activeDevice) {
      // Security Alert: Token Rotation Security
      user.devices = [];
      await user.save();
      return res.status(403).json({ success: false, message: "Security Breach! All devices logged out." });
    }
